...
def get_operators_config(objects: Dict) -> Dict:
    ...
    for robot in ROBOTS:
        ...
        robot["zone"] = get_zone_for_position(robot["init_pos"])

    for human in HUMANS:
        human["init_pos"] = __get_operator_init_position(human, doors)
        human["zone"] = get_zone_for_position(human["init_pos"])
...
def __get_operator_init_position(data: Dict, doors: List[Dict]) -> tuple:
    ...
    # calculate pos_x
    if data["side"] == "left":
        pos_x = entry_door["init_pos"][0] + entry_door["size"][0] 
    else:
        pos_x = entry_door["init_pos"][0] - data["size"][0]
    # calculate pos_y
    pos_y = entry_door["init_pos"][1] - data["size"][1]
    return (pos_x, pos_y)
