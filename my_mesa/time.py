...
class BaseScheduler:
    ...
    It assumes that each agent added has a `step` method which takes no arguments and executes the agent's micro-actions.
...
class SimultaneousActivation(BaseScheduler):
    ...
    - The `advance` method then applies these changes, simulating simultaneous micro-action.
...
class StagedActivation(BaseScheduler):
    ...
    This scheduler is useful for complex models where micro-actions need to be broken down into distinct phases
